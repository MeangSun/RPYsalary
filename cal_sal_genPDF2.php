...
class PDF extends FPDF {
	...
	function calculate() {
		...
		//$food=$TotalWorkExtra*(-30);
		/*
$food=$num_food*(-15);
		if($EMP_TYPE!=5) {
			$this->SetXY(116, 186+(8*$i));
			$this->Cell( 60  , 8 ,iconv( 'UTF-8','cp874' , "ค่าอาหาร -15 x ".$num_food)  , 1,0,'C');
...
/* 		$this->SetXY(116, 186+(8*$i)); 
		...
		if ($EMP_TYPE==1 || $EMP_TYPE==2 || $EMP_TYPE==4  || $EMP_TYPE==6) {
			$this->SetXY(15, 202+(8*$j));
			$this->Cell( 50  , 8 ,iconv( 'UTF-8','cp874' , "หักค่าไม่มีใบลา / อื่นๆ")  , 1,0,'C');
		...
		$TotalPayText = number_format(($TotalSalary))
			...
			.(($ETC==0)?"":(($ETC>=0)?" + ".number_format($ETC):" - ".number_format($ETC*-1)));
		...
		$this->SetFont('cordia','',18);
		$this->SetXY(110, 270);
		$this->Cell( 100  , 6 ,iconv( 'UTF-8','cp874' ,"ลงชื่อ ______________________________ รับทราบ")  , 0,0,'L');
